...
class _HandleState extends State<Handle> {
  ...
  void _onDragUpdated(Offset pointer) {
    ...
    final context = _list?.context;
    print(
        'onDragUpdated called, scrollController: $scrollController, context: $context');
    if (scrollController != null && context != null) {
      final renderBox = context.findRenderObject() as RenderBox?;
      if (renderBox != null) {
        final height = renderBox.size.height;
        final localPosition = renderBox.globalToLocal(pointer);
        const edgeHeight = 80.0;
        const scrollSpeed = 14.0;

        print('localPosition.dy: ${localPosition.dy} height: $height');
        if (localPosition.dy < edgeHeight) {
        ...
        } else if (localPosition.dy > height - edgeHeight) {
          scrollController.animateTo(
            (scrollController.offset + scrollSpeed)
                .clamp(0.0, scrollController.position.maxScrollExtent),
            duration: const Duration(milliseconds: 16),
